...
def get_arrival_flights():
    ...
    try:
        ...
        for v in items:
            ...
            remark = v.get("remark", "")
            master_raw = v.get("masterFlightId")
            master_id = str(master_raw).strip() if master_raw is not None else ""
            actual_flight = master_id if master_id and master_id != "None" else flight_id
