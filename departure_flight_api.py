...
URL = "https://apis.data.go.kr/B551177/statusOfAllFltDeOdp/getFltDeparturesDeOdp"
...
def get_departure_flights():
    """
    공공데이터포털의 출국(출발) 여객기 운항정보 API를 호출하여 DataFrame 형태로 반환합니다.
    """
    ...
    params = {
        ...
        "searchFrom": "0000",
        "searchTo": "2400",
        "passengerOrCargo": "P",  # 여객기만 (대문자 P 기준)
    ...
    try:
        ...
        for v in items:
            ...
            chkin_range = v.get("chkinRange", "미정")
            gate_no = v.get("gateNumber", "미정")
...
if __name__ == "__main__":
    ...
    print("🛫 실시간 인천공항 출국(출발) 여객기 정보 조회 중...\n")
    df_departures = get_departure_flights()
    ...
    if df_departures is not None and not df_departures.empty:
        ...
        print(f"⏰ [현재 시간 기준]: {current_time_hm} 이후 출발 스케줄\n")
        
        # 2. 현재 시간 이후의 항공편만 필터링
        df_valid = df_departures[df_departures["예정시간"] != "N/A"]
        ...
        print("\n💡 [출국 현황 요약 (전체 데이터 기준)]")
        total = len(df_valid)
        delayed = df_valid[df_valid['상태'].str.contains('지연', na=False)]
        departed = df_valid[df_valid['상태'].str.contains('출발', na=False)]
        print(f"👉 전체 조회된 여객기 수: {total} 편")
        print(f"👉 '출발 완료' 처리된 여객기: {len(departed)} 편")
        print(f"👉 '지연' 상태인 여객기: {len(delayed)} 편")
    else:
        print("출국 정보를 불러오지 못했습니다.")
